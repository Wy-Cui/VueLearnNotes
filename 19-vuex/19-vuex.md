...
并且在cli3中可手动选择是否创建vue-router和vuex等
...
如果不是父子组件如何通信，此时就需要vuex了，还是这两个组件，现在不使用父子通信，直接使用vuex。

**3.使用vuex管理状态**

- 使用`npm install vuex --save`安装Vuex（如果是CLI3可直接选择自动安装）
...
Vuex 使用**单一状态树(单一数据源)**——是的，用一个对象就包含了全部的应用层级状态。至此它便作为一个“唯一数据源 ([SSOT](https://en.wikipedia.org/wiki/Single_source_of_truth))”而存在。。这也意味着，每个应用将仅仅包含一个 store 实例。单一状态树让我们能够直接地定位任一特定的状态片段，在调试的过程中也能轻易地取得整个当前应用状态的快照。
...
例如有这样一个store实例，我们需要获取**年龄大于20岁的学生**：
...
如果很多组件中需要**年龄大于20岁的学生**，你可能会将这个计算属性复制，将filter函数写很多遍，但是如果你有**Getters**。
...
```
<h4>年龄大于20岁的学生：{{getStu}}</h4>
```

> 或者不需要设置计算属性，直接在template中使用

```
<h4>年龄大于20岁的学生：{{$store.getters.getStudents}}</h4>
```

现在只需要调用getters的getStudents对象，就能获取数据。



此时如果有一个需求，获取**年龄大于20岁的学生数量**，我们只需在filter()后面加length就行，但是重新创建一个计算属性，filter前面会有很多重复代码，如何简化代码形式呢？

> 在store实例的getters中定义getStudentsCount函数

```JS
getters: {
    //getters的基本使用
    //在getters对象里面建议把powerCounter之类的函数写成对象的增强写法，不用写成箭头函数，因为这种写法更清晰
    //1.获取count的平方
    powerCount(state) {
    ...
    //3.获取年龄大于20岁的学生数量
    //我们定义的getters函数中的第二个参数是getters对象本身，可以获取到getters里面定义的函数
...
```
<h4>查询指定ID（传入ID）的学生信息:{{stuById}}</h4>
```

> 或者直接在template中使用

```
<h4>查询指定ID（传入ID）的学生信息:{{$store.getters.getStuById(110)}}</h4>
```
...
```js
addCount (counter) {
    this.$store.commit('addCount', counter) // 将counter传入
}
```
...
```js
mutations: { 
    addCount (state, counter) { // 第二个参数是counter，第一个始终是state
        state.count += counter
    }
}
```
...
1. 普通提交风格
   ...
   ```js
   this.$store.commot('increment', counter)
   ```

   此时counter传过去的就是counter=10，一个值
...
此时counter传过去是一个对象payload（载荷）
   
   ```js
   //index.js
   addCount (state, payload) { // 此时传入的就不是一个counter值了，而是一个对象，所以这里把counter换成payload更好
...
2. 如果数据在state已经初始化了，比如修改某一属性值（更改姓名），则可以使用直接赋值方式；如果数据是需要添加进来的话，比如添加一个年龄属性，不能直接使用赋值方式。
...
3. 关于删除属性
   ...
   ```js
         ...
         Vue.delete(state.user, 'age')// 响应式删除age,注意age要加单引号
   ```
